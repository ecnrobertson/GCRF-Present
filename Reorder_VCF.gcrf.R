...
vcf <- read_tsv("~/Desktop/GCRF/GCRF-Present/GenomicPipeline/data/bslmm/GCRF_bill_PC.bslmm.param.txt", comment = "##", progress = FALSE) %>% rename(CHROM = chr, POS=ps)
...
dim(zf_ified)
zf_ified %>% dplyr::select(ZFCHROM,SNP,ZFPOS) %>% write.table("~/Documents/CurrentProjects/UCLA_UCSC/BGP/PABU/GWAS/GEMMA_inputfile/PABU_clean.SNP.rm_hets.RegionFID.imuted_gt.fix_name.new_pheno_inclLA.ZFchr.map",row.names=F,quote=F,sep="\t")
...
zf_ified %>% 
  ...
  write.delim("~/Documents/CurrentProjects/UCLA_UCSC/BGP/PABU/GWAS/GEMMA_inputfile/PABU_clean.SNP.rm_hets.RegionFID.imuted_gt.fix_name.new_pheno_inclLA.noNA.chr1-31ZF.012.mrMLM.genotype.txt", quote = FALSE, sep = "\t")
